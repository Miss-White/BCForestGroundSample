...
setMethod(
  ...
  definition = function(treeMC){
    worktable <- treeMC[,.(CLSTR_ID, HEIGHT, BA_TREE, PLOT_WT, ST_FL, LV_D, BTOP, SAMP_TYP)]
    rm(treeMC)
    ...
    stand <- worktable[HEIGHT %>>% 0 & ST_FL == "S",
    ...
    standlive <- worktable[HEIGHT %>>% 0 & ST_FL == "S" & LV_D == "L",
                           .(WTH1 = sum(PLOT_WT),
    ...
    standliveNONBTOP <- worktable[HEIGHT %>>% 0 & ST_FL == "S" & LV_D == "L" & is.na(BTOP),
                                  .(WTH2 = sum(PLOT_WT),
