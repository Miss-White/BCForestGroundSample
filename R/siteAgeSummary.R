...
setGeneric("siteAgeSummary",
           function(cpldSiteAgeData, siteToolsDLLPath, sasExePath) {
...
setMethod(
  ...
  definition = function(cpldSiteAgeData, siteToolsDLLPath, sasExePath){
    ...
    ahs_txo <- rbind(tlsxo[,.(BGC_ZONE = unique(BGC_ZONE),
                           ...
                           by = "CLSTR_ID"],
                     tlsxo[,.(BGC_ZONE = unique(BGC_ZONE),
                              AGEB_TXO = mean(AGE_BH, na.rm = TRUE),
                              AGET_TXO = mean(AGE_TOT, na.rm = TRUE),
                              HT_TXO = mean(HEIGHT, na.rm = TRUE),
                              SI_M_TXO = mean(SI_TREE, na.rm = TRUE),
    ...
    ahs_msp[, ':='(BEC_I_C = BEC2IC(BGC_ZONE),
                   STSpecies = siteToolsSpeciesConvertor(SPECIES))]
    ahs_msp[BEC_I_C == "?", BEC_I_C := "I"]
    ahs_msp[AGEB_TLS > 0 & HT_TLS > 0,
            SI_C_TLS := SiteTools_HTBoredAge2SI(boredAge = AGEB_TLS, height = HT_TLS, species = STSpecies,
                                                ICRegion = BEC_I_C, ageType = 1, estimateMethod = 1,
...
setMethod(
  ...
  signature = c(cpldSiteAgeData = "data.table",
                siteToolsDLLPath = "character",
                sasExePath = "missing"),
  definition = function(cpldSiteAgeData, siteToolsDLLPath){
    return(siteAgeSummary(cpldSiteAgeData, siteToolsDLLPath, 
