...
setGeneric("boredAgeCalculator_Crted",
           function(boredAge, boredHeight, treeHeight, 
                    species, ICRegion, siteToolsDLLPath, sasExePath) {
  ...
  definition = function(boredAge, boredHeight, treeHeight, 
    ...
    while(nrow(worktable) > 0){
      ...
      worktable[firstBHA > 0, SI_N := SiteTools_HTBoredAge2SI(boredAge = firstBHA, height = HT_TREE, 
                                                              species = speciesFRED, ICRegion = BEC_I_C, 
                                                              ageType = 1, estimateMethod = 1,
                                                              siteToolsDLLPath = siteToolsDLLPath,
                                                              sasExePath = sasExePath)]
      ## call SI_Y2BH an equivalent function to sindex_y2bh.sas
      worktable[firstBHA > 0, CORR_N := SiteTools_Y2BH(species = speciesFRED, ICRegion = BEC_I_C,
                                                       siteIndex = SI_N,
                                                       siteToolsDLLPath = siteToolsDLLPath,
                                                       sasExePath = sasExePath)]
      ## update bored age 
...
setMethod(
  ...
  signature = c(boredAge = "numeric",
                ...
                sasExePath = "missing"),
  definition = function(boredAge, boredHeight, treeHeight, 
                        species, ICRegion, siteToolsDLLPath){
    return(boredAgeCalculator_Crted(boredAge, boredHeight, treeHeight, 
                                    species, ICRegion, siteToolsDLLPath,
