...
setMethod(
  ...
  definition = function(smallTreeData, smallTreePlotHeader){
    clustersummaries <- unique(smallTreePlotHeader[,.(CLSTR_ID, PROJ_ID, SAMP_NO, TYPE_CD)],
                               by = "CLSTR_ID")
    sm_tr <- merge_dupUpdate(smallTreeData,
                             smallTreePlotHeader,
                             by = c("CLSTR_ID", "PLOT"),
                             all.x = TRUE)
    ...
    smtr_cs <- sm_tr[,.(PROJ_ID = unique(PROJ_ID),
                        SAMP_NO = unique(SAMP_NO),
                        TYPE_CD = unique(TYPE_CD),
                        SMTR_CT1 = sum(SMTR_CT1),
